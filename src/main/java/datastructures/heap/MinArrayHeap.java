...
public class MinArrayHeap {
  ...
  public void insert(int item) {
    ...
    while (lastIndex >= 0
        && minHeap.get(lastIndex) < minHeap.get((int) Math.floor((lastIndex - 1) / 2))) {
  ...
  public int remove() {
    ...
    if (minHeap.size() > 1) {
      ...
      // Continue swapping until the parent is smaller than both children or you reach the last
      // level in the tree.
  ...
  public String toString() {
    ...
    minHeap.forEach(e -> stringBuilder.append(",").append(e.toString()));
    return "MinArrayHeap{arrayList=[" + stringBuilder.toString().substring(1) + "]}";
  }
}
